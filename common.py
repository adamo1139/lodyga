...
def get_mt_bench_results(result_file):
    
    headers=["Model","pl_wynik", "odpowiedzi_pl", "Kodowanie", "Ekstrakcja", "Humanistyka", "Matematyka", "Wnioskowanie", "Odgrywanie ról", "Nauki ścisłe", "Piśmiennictwo"]

    df = pd.read_csv(result_file, skiprows=1, header=None, names=headers,  sep=";")

    # liczymy średnią
    df['Średni Wynik'] = df[['Kodowanie', "Ekstrakcja", "Humanistyka", "Matematyka", "Wnioskowanie", "Odgrywanie ról", "Nauki ścisłe", "Piśmiennictwo"]].mean(axis=1).round(2)

    # Przesunięcie kolumny 'AVG' na drugie miejsce
    columns = df.columns.tolist()
    columns.insert(3, columns.pop(-1))

    # Utworzenie nowej ramki danych z przesuniętą kolumną 'AVG'
    df = df[columns]

    # sortowanie po AVG
    df = df.sort_values(by='pl_wynik', ascending=False)
